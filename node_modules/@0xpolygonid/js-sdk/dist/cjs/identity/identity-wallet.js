...
class IdentityWallet {
    ...
    async createProfile(did, nonce, verifier) {
        ...
        const profile = js_iden3_core_1.Id.profileId(id, BigInt(nonce));
        const profileDID = js_iden3_core_1.DID.parseFromId(profile);
    ...
    async getDIDTreeModel(did) {
        const claimsTree = await this._storage.mt.getMerkleTreeByIdentifierAndType(did.toString(), mt_1.MerkleTreeType.Claims);
        const revocationTree = await this._storage.mt.getMerkleTreeByIdentifierAndType(did.toString(), mt_1.MerkleTreeType.Revocations);
        const rootsTree = await this._storage.mt.getMerkleTreeByIdentifierAndType(did.toString(), mt_1.MerkleTreeType.Roots);
    ...
    async addCredentialsToMerkleTree(credentials, issuerDID) {
        ...
        for (let index = 0; index < credentials.length; index++) {
            ...
            await this._storage.mt.addToMerkleTree(issuerDID.toString(), mt_1.MerkleTreeType.Claims, coreClaim.hIndex(), coreClaim.hValue());
        }
        ...
        await this._storage.mt.addToMerkleTree(issuerDID.toString(), mt_1.MerkleTreeType.Roots, claimTreeRoot.bigInt(), BigInt(0));
