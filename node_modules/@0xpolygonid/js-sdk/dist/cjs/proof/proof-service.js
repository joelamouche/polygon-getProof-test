...
class ProofService {
    ...
    async generateProof(proofReq, identifier, credential, opts) {
        ...
        const { inputs, vp } = await this.generateInputs(preparedCredential, identifier, proofReq, opts);
    ...
    async generateMTPV2Inputs(preparedCredential, identifier, proofReq, opts) {
    ...
    async generateMTPV2OnChainInputs(preparedCredential, identifier, proofReq, opts) {
        const circuitClaimData = await this.newCircuitClaimData(preparedCredential.credential, preparedCredential.credentialCoreClaim);
        ...
        circuitInputs.id = identifier.id;
        circuitInputs.requestID = BigInt(proofReq.id);
        const stateProof = await this._stateStorage.getGISTProof(identifier.id.bigInt());
        ...
        const signature = await this._identityWallet.signChallenge(opts.challenge ?? BigInt(proofReq.id), authPrepared.authCredential);
        circuitInputs.signature = signature;
    ...
    async generateQuerySigV2Inputs(preparedCredential, identifier, proofReq, opts) {
        const circuitClaimData = await this.newCircuitClaimData(preparedCredential.credential, preparedCredential.credentialCoreClaim);
        circuitClaimData.nonRevProof = (0, common_1.toClaimNonRevStatus)(preparedCredential.revStatus);
    ...
    async generateQuerySigV2OnChainInputs(preparedCredential, identifier, proofReq, opts) {
        const circuitClaimData = await this.newCircuitClaimData(preparedCredential.credential, preparedCredential.credentialCoreClaim);
        const authPrepared = await this.prepareAuthBJJCredential(identifier);
        const authClaimData = await this.newCircuitClaimData(authPrepared.authCredential, authPrepared.authCoreClaim);
        circuitClaimData.nonRevProof = (0, common_1.toClaimNonRevStatus)(preparedCredential.revStatus);
    ...
    async newCircuitClaimData(credential, coreClaim) {
        ...
        if (sigProof) {
            ...
            const issuer = js_iden3_core_1.DID.parse(sigProof.issuerData.id);
            const rs = await this._credentialWallet.getRevocationStatus(sigProof.issuerData.credentialStatus, issuer, sigProof.issuerData);
