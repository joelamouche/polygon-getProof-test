...
import { ErrPackedWithUnsupportedCircuit, ErrProofIsInvalid, ErrSenderNotUsedTokenCreation, ErrStateVerificationFailed, ErrUnknownCircuitID } from '../errors';
...
export class DataPrepareHandlerFunc {
     ...
     */
    prepare(hash, did, profileNonce, circuitId) {
...
export class ZKPPacker {
    ...
    async pack(payload, params) {
        ...
        const { provingKey, wasm, dataPreparer } = this.provingParamsMap.get(params.provingMethodAlg.toString());
    ...
    async unpack(envelope) {
        ...
        const { key: verificationKey, verificationFn } = this.verificationParamsMap.get(provingMethodAlg.toString());
        if (!verificationKey) {
        ...
        const verificationResult = await verificationFn.verify(token.circuitId, token.zkProof.pub_signals);
